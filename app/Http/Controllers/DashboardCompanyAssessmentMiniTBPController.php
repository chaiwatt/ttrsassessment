...
class DashboardCompanyAssessmentMiniTBPController extends Controller
{
    public function Index(){
        ...
        // return $businessplan;
        return view('dashboard.company.assessment.minitbp.index')->withMinitbps($minitbps);
    }
    public function Edit($id){
        ...
        return view('dashboard.company.assessment.minitbp.edit')->withMinitbp($minitbp)
                                                ->withBanks($banks)
                                                ->withCompany($company)
    ...
    public function Pdf(){
        ...
        $pdf = Pdf::loadView('dashboard.company.assessment.minitbp.pdf',$data);
    ...
    public function EditSave(EditMiniTbpRequest $request,$id){
        ...
        return  redirect()->route('dashboard.company.assessment.minitbp')->withSuccess('แก้ไขรายการสำเร็จ');
    }
    ...
    public function Submit($id){
        $minitbp = MiniTBP::find($id);
        return view('dashboard.company.assessment.minitbp.submit')->withMinitbp($minitbp);
    }
    public function SubmitSave(Request $request, $id){
        ...
        EmailBox::send(User::where('user_type_id',7)->first()->email,'TTRS:ส่งเอกสาร Mini TBP','เรียน Master<br> '. Company::where('user_id',Auth::user()->id)->first()->name . ' ได้ส่งเอกสาร Mini TPB โปรดตรวจสอบ/Assign ผู้รับผิดชอบ ได้ที่ <a href='.route('dashboard.admin.assessment.projectassignment').'>คลิกที่นี่</a> <br>ด้วยความนับถือ<br>TTRS');
        return redirect()->route('dashboard.company.assessment.minitbp')->withSuccess('ส่งเอกสาร mini TBP สำเร็จ');
    }
}
