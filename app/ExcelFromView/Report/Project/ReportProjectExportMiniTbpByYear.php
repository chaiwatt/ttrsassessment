...
class ReportProjectExportMiniTbpByYear implements FromView,ShouldAutoSize,WithTitle
{
    ...
    public function view(): View
    {
        if($this->year == 0){
            ...
            $fulltbps = FullTbp::whereIn('mini_tbp_id',$minitbparr)->orderBy('fulltbp_code','asc')->orderBy('fulltbp_code','asc')->get();
        }else{
            $minitbparray = MiniTBP::whereNotNull('submitdate')->whereYear('submitdate',$this->year)->pluck('id')->toArray();
            $fulltbps = FullTbp::whereIn('mini_tbp_id', $minitbparray)->orderBy('fulltbp_code','asc')->orderBy('fulltbp_code','asc')->get();
        }
