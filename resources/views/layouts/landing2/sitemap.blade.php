...
<head>
    ...
    {{-- <div class="main-content"> --}}
        ...
        <div class="rs-inner-blog pt-80 pb-120 md-pt-90 md-pb-90">
            <div class="container">
                <div class="row">
                                                ...
                                                <div class="col-sm-12 col-lg-6">
                                                    ...
                                                    @php
                                                        ...
                                                        if(Config::get('app.locale') == 'en'){
                                                            $menuname = $menu->engname;
                                                        }
                                                    @endphp
                                                                   
                                                        <div class="mb-3">
                                                            <h6 class="font-weight-semibold" style="font-family: kanit">
                                                              @php
                                                                  $mainlink = $menu->url;
                                                                  if($key == 0 ){
                                                                    $mainlink = url('');
                                                                  }else if(substr("$menu->url",0,1) == '#'){
                                                                    $mainlink = url('') . '/' . $menu->url;
                                                                  }
                                                                  if($menu->name == "เข้าสู่ระบบ"){
                                                                    $mainlink = url('') . '/register' ;
                                                                  }
                                                                 
                                                              @endphp
                                                              @if ($menu->menu_type_id <=2)
                                                                <a href="{{$mainlink}}" class="text-info">{{$menuname}}</a>
                                                              @endif
                                                                
                                                            </h6>

                                                                @if (count($directmenus2->where('submenu',$menu->id)) > 0)
                                                                    <ul class="list list-unstyled " style="margin-top: -15px; padding-left:10px">
                                                                        @foreach ($directmenus2->where('submenu',$menu->id) as $submenu)
                                                                        @php
                                                                            $submenuname = $submenu->name;
                                                                            if(Config::get('app.locale') == 'en'){
                                                                                $menuname = $submenu->engname;
                                                                            }
                                                                        @endphp
                                                                            @php
                                                                                $sublink = $submenu->url;
                                                                                if(substr("$submenu->url",0,1) == '#'){
                                                                ...
                                                                @if ($menu->url == "#rs-blog")
                                                                    ...
                                                                    <ul class="list list-unstyled " style="margin-top: -15px;padding-left:10px">
                                                                        @foreach ($blogs as $blog)
                                                                            <li><a href="{{route('landing.page',['slug' => $blog->slug])}}">{{$blog->name}}</a></li>
                                                                        @endforeach
                                                                    </ul>
                                                                    @endif
