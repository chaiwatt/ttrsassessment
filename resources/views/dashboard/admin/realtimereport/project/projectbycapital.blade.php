        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{route('dashboard.admin.realtimereport.project.getfulltbpbyyearbudget')}}" method="get">
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
									<div class="form-group">
										...
										<select name="projectbudget" data-placeholder="ปี" value="{{old('projectbudget')}}"  class="form-control form-control-lg form-control-select2">
											@foreach ($projectbudgets as $key => $projectbudget)
                                                @if ($key == 0)
                                                        <option value="{{$projectbudget->id}}" @if ($projectbudget->id == Request::get('projectbudget')) selected @endif > น้อยกว่า {{$projectbudget->minbudget}}</option> 
                                                   @elseif($key == ($projectbudgets->count()-1))
                                                        <option value="{{$projectbudget->id}}" @if ($projectbudget->id == Request::get('projectbudget')) selected @endif > มากกว่า {{$projectbudget->maxbudget}}</option> 
                                                   @else    
                                                        <option value="{{$projectbudget->id}}" @if ($projectbudget->id == Request::get('projectbudget')) selected @endif > ตัั้งแต่ {{$projectbudget->minbudget}} - {{$projectbudget->maxbudget}}</option> 
                                                @endif
