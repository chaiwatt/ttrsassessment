        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <a href="{{route('setting.admin.website.page')}}" class="breadcrumb-item"> หน้าประกาศ</a>
    ...
    <div class="content">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{route('setting.admin.website.announce.editsave',['id' => $announce->id])}}" enctype="multipart/form-data">
                            @csrf
                            <div class="row">	
                                <div class="col-md-12">
                                    <fieldset>	
                                        <div class="form-group">
                                            ...
                                            <select name="announcecategory" placeholder="เมนู" class="form-control form-control-select2">
                                                ...
                                                @foreach ($announcecategories as $announcecategory)
                                                    <option value="{{$announcecategory->id}}"
                                                        @if ($announcecategory->id == $announce->announce_category_id)
                                                            selected
                                                        @endif
                                                        >{{$announcecategory->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>หัวเรื่อง<span class="text-danger">*</span></label>
                                            <input type="text"  name="title" value="{{$announce->title}}"  placeholder="หัวเรื่อง" class="form-control form-control-lg">
                                        </div>
                                        <div class="form-group">
                                            <label>ข้อมูลประกาศ<span class="text-danger">*</span></label>
											<textarea name="content" id="summernote" class="form-control mb-3" rows="7" cols="1" placeholder="บทความ">{{$announce->content}}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <div id="input_attachment_wrapper">
                                                @foreach ($announceattachments as $announceattachment)
                                                    <input name="input_attachment[]" value="{{$announceattachment->id}}" data-id="{{$announceattachment->id}}" class="input_attachment" hidden>        
                                                @endforeach   
                                            </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="status" placeholder="สถานะการแสดง" class="form-control form-control-select2">
                                                @foreach ($pagestatuses as $pagestatus)
                                                    <option value="{{$pagestatus->id}}"
                                                        @if ($pagestatus->id == $announce->page_status_id)
                                                            selected
                                                        @endif
                                                        >{{$pagestatus->name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </fieldset>
